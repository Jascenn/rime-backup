...
#   open 序号      - 打开指定序号的备份文件夹 (例如: ./simple_backup.sh open 1)
#   备份名称        - 使用指定名称创建备份 (例如: ./simple_backup.sh '修改配置前')
# 自动判断不同设备的Rime设置文件夹
detect_rime_dir() {
    # 检测操作系统类型
...
if [ "$1" = "list" ]; then
    ...
    echo ""
    echo "提示: 使用 './simple_backup.sh open 序号' 可打开指定备份文件夹"
    if [ "$show_full_path" = false ]; then
        echo "提示: 使用 './simple_backup.sh list full' 可查看备份完整路径"
    fi
...
if [ "$1" = "open" ]; then
    if [ -z "$2" ]; then
        echo "错误: 未指定备份序号。用法: ./simple_backup.sh open <序号>"
        exit 1
    fi
    ...
    if [ -z "$backup_path" ]; then
        echo "错误: 未找到序号 $2 对应的备份。请运行 './simple_backup.sh list' 查看可用备份列表。"
        exit 1
    fi
...
echo "！提示：运行 './simple_backup.sh list' 查看所有备份"
echo "！提示：运行 './simple_backup.sh clean' 清理旧备份"
echo "！提示：运行 './simple_backup.sh 备份名称' 创建带自定义名称的备份"
